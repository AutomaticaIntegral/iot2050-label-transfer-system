...
# Script para crear backup completo del IoT2050 CON PASSWORD
# Cliente: ADISSEO - Automática Integral

echo "🔄 BACKUP DEL IOT2050 → MAC (CON PASSWORD)"
echo "📅 $(date)"
echo "=" $(printf '=%.0s' {1..60})
...
# Verificar conexión SSH (permitiendo password)
echo "🔍 Verificando conexión SSH..."
echo "⚠️ Te pedirá el password de root del IoT2050:"

if ! ssh -o ConnectTimeout=10 $IOT_USER@$IOT_IP "echo 'Conexión OK'"; then
...
echo "⚠️ Te pedirá el password varias veces durante el proceso:"
echo ""

# Parar servicio
echo "⏸️ Parando servicio..."
ssh -o ConnectTimeout=10 $IOT_USER@$IOT_IP "sudo systemctl stop $IOT_SERVICE"

# Crear backup comprimido
echo "📦 Creando backup comprimido..."
ssh -o ConnectTimeout=10 $IOT_USER@$IOT_IP << EOF
...
ssh -o ConnectTimeout=10 $IOT_USER@$IOT_IP "sudo systemctl start $IOT_SERVICE"

# Transferir archivos a Mac
echo ""
echo "📥 Descargando backup a Mac..."
echo "⚠️ Te pedirá el password algunas veces más:"
...
echo ""
echo "💡 RECOMENDACIÓN:"
echo "   🔐 Para evitar escribir password repetidamente:"
